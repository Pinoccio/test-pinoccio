...
bool flash256RFR2() {
  ...
  if (pgm.foundSignature() != -1) {
    ...
    pgm.eraseChip();
    TD(Serial1.println("-- writing fuses"));
    pgm.writeFuseBytes(0xDE, 0xD0, 0xFE, 0xFF);
...
bool checkPinVia328(int scoutPin, int avr328Pin) {
  int offset = 1;
  int offset2 = 13;
  if (scoutPin >= 10) {
    offset = 2;
...
bool checkPinViaDriver(int scoutPin, int driverPin) {
  int offset = 1;
  int offset2 = 13;
  if (scoutPin >= 10) {
    offset = 2;
...
bool expectPrompt(int timeout) {
  ...
  while (millis() - time < timeout) {
    if (Serial.available() > 0) {
      debug ? TD(Serial1.println("Serial is available")) : false;
      serialFound = true;
      break;
    }
  }
  ...
  debug ? TD(Serial1.print("Reading: ")) : false;
  while (millis() - time < timeout && i<256) {
    if (Serial.available() > 0) {
      buf[i++] = Serial.read();
      debug ? TD(Serial1.write(buf[i-1])) : false;
