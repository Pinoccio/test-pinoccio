...
void flash256RFR2() {
  ...
  if (pgm2.foundSignature() != -1) {
    ...
    // now flash
    err = pgm2.writeProgramFromSerialFlash(0x00000, &DriverFlash, DRIVER_FLASH_CS, 0x40000, 60031); // be sure not to make this too short! Don't truncate
  }
...
bool expectPrompt(int timeout) {
  ...
  while (millis() - time < timeout && i < 21) {
